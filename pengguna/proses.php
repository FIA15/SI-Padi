...
if(isset($_POST['add'])) {
    ...
    if(mysqli_num_rows($ceknip) > 0){
        echo "<script>alert('NIP $nip Sudah Terdaftar!');window.location='add.php';</script>";
    } else {
        mysqli_query($con, "INSERT INTO user (user_id, id_kecamatan, id_desa, username, password, nip, level) 
        VALUES ('$uuid', '$kecamatan', '$desa', '$nip', '$password', '$nip', '$level')") or die(mysqli_error($con));
    }
    ...
    $input = "UPDATE user SET  nip = '$nip',  id_kecamatan = '$kecamatan',  id_desa = '$desa',  username = '$username', 
            password = '$password', level = '$level' WHERE user_id = '$id'";
    $input2 = "UPDATE user SET  nip = '$nip',  id_kecamatan = '$kecamatan',  id_desa = '$desa',  username = '$username', 
